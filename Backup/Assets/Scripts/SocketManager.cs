...
public class SocketManager
{
    ...
    private void _onReceiveSocket()
    {
        while (true)
        {
            ...
            try
            {
                ...
                if (receiveLength > 0)
                {
                    Gaming.Instance.onRecv(_tmpReceiveBuff, receiveLength);
    ...
    private void SendMsgBase(byte[] _data)
    {
        if (clientSocket == null || !clientSocket.Connected)
        {
            _ReConnect();
        ...
        clientSocket.BeginSend(_data, 0, _data.Length, SocketFlags.None, new AsyncCallback(_onSendMsg), clientSocket);
    }
